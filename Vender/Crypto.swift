...
//        print("md5 String: \(self)")
        ...
        let original_str = (self.data(using: String.Encoding.utf8)! as NSData).bytes
        
        var result: [CUnsignedChar] = [CUnsignedChar](repeating: 0, count: Int(CC_MD5_DIGEST_LENGTH))
        CC_MD5(original_str, CC_LONG((self.data(using: String.Encoding.utf8)! as NSData).length), &result)
